    ...
    <div>
        <div class="card mb-4">
            <div class="card-body">
                ...
                <h5 class="card-title">
                    발주 정보
                    <span class="badge badge-info">발주상태: {{ shippingStatus === 'standby' ? '발주 대기' : '발주 완료' }}</span>
                    <span class="badge badge-success">입고상태: {{ inputStatus === 'standby' ? '입고 대기' : '입고 완료' }}</span>
                </h5>
    ...
    export default {
        ...
        methods: {
            ...
            submitForm: function () {
                ...
                if (!this.selectedSupplier || !this.salesManId) {
                    toastr.error('필수 입력 사항들을 입력하세요.');
                    return false;
                }

                if (this.orderProducts.length === 0) {
                    toastr.error('발주상품이 없습니다.');
                    return false;
                }
