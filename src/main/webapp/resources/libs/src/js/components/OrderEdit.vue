                ...
                <h5 class="card-title">
                    ...
                    <template v-if="returnStatus !== ''">
                        ...
                        <span class="text-warning">
                            반품상태:
                            <template v-if="returnStatus === 'progress'">
                                반품접수
                            </template>
                            <template v-else-if="returnStatus === 'completed'">
                                반품완료
                            </template>
                        </span>
                    </template>
...
<!--                    </tr>-->
                        ...
                        <td>
                            <div class="input-group input-group-sm">
                                ...
                                <div class="input-group-append">
                                    <span class="input-group-text" @click="showDatepicker">
                ...
                <table class="table table-sm text-center table-middle table-hover mb-5">
                    ...
                    <tr>
                        ...
                        <th>단가(원)</th>
                        <th>합계금액(원)</th>
                        <th v-if="returnStatus === '' && (orderStatus !== 'canceled' && releaseStatus === 'progress')">삭제</th>
                    </tr>
                    </thead>
                    <tbody>
                    <tr v-for="(orderProduct, index) in orderProducts">
                        ...
                        <td v-if="returnStatus === '' && (orderStatus !== 'canceled' && releaseStatus === 'progress')">
                    ...
                    <tr>
                        ...
                        <td></td>
                        <td>{{ Number(total_amount).toLocaleString() }}</td>
                        <td v-if="returnStatus === '' && (orderStatus !== 'canceled' && releaseStatus === 'progress')"></td>
                    </tr>
                    </tfoot>
                </table>
                <template v-if="returnStatus !== ''">
    ...
    export default {
        ...
        methods: {
            ...
            submitForm: function () {
                ...
                if (!this.selected_shop || !this.selected_deliverer_id) {
                    toastr.error('필수 입력 사항들을 입력하세요.');
                    return false;
                }

                if (this.orderProducts.length === 0) {
                    toastr.error('주문상품이 없습니다.');
                    return false;
                }
                let gt0 = true;
            ...
            receiveReturn: function () {
                ...
                this.orderProducts.forEach(orderProduct => {
                    ...
                    this.$set(orderProduct, 'productId', orderProduct.product.id);
                })
